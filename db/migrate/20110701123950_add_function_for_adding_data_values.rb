class AddFunctionForAddingDataValues < ActiveRecord::Migration
  def self.up
    execute <<-SQL
          CREATE OR REPLACE FUNCTION clear_datacolumn_accepted_values(datacolumn_id integer)
            RETURNS boolean AS
          $BODY$update sheetcells
            ...
            COST 100;
          ALTER FUNCTION clear_datacolumn_accepted_values(integer) OWNER TO postgres;
    SQL
    ...
    execute <<-SQL
      CREATE OR REPLACE FUNCTION isyear("year" text)
        RETURNS boolean AS
        'select 1=1'
        LANGUAGE sql VOLATILE
        COST 100;
        ALTER FUNCTION isyear(text) OWNER TO postgres;
    SQL
      ...
      CREATE OR REPLACE FUNCTION accept_datacolumn_values(datatype_id integer, datacolumn_id integer, datagroup_id integer, system_datagroup_id integer)
        ...
        $BODY$          -- regardless of the data type we search for portal matches first
                  ...
                  and ((datatype_id = 7 and isnumeric(sheetcells.import_value) = true)
                    or (datatype_id = 2 and isyear(sheetcells.import_value) = true)
                  ...
                  update sheetcells
                  set category_id = insert_category(sheetcells.import_value, sheetcells.import_value, $3, 4),
                    accepted_value = null,
                    updated_at = now(),
                    datatype_id = 5, -- whatever the original data type this is now a category data type
                    status_id = 5
