...
class ImportController < ApplicationController
  ...
  #! somewhere should be the question, if users are logged in or not.
  #? before_filter :require_user, :only => [:log_in]

  # The index method of the import controller collects all data sets
  # provided to the portal (see Context).  At this stage we assume
  # that the general metadata was already uploaded to the portal.  !!
  # We should save files directly when uploading general metadata and
  # then redirect to the "upload - edit" organized here.
  def index
    @contexts = Context.all
  end

  # Searching contexts uses "ActsAsFerret" to search one or several keywords
  # in Context or Method information.  It finds Contexts, in which the keywords
  # appear.  This is done by building at the time of Context import.
  def search_contexts
#    session[:ch_method_list] = []
    @query = params[:q]
    @results = ActsAsFerret::find(@query || "search", [Context])
  end

  # Is not used at the moment but could be used to step throuth the
  # data headers of a context.  In the Multi-Step railscast, this is
  ...
  # currently not used, was thought to step through the import routine
  def step_raw_data
    render :partial => "step_raw_data"
  end
  ...
  def create_dataset_filevalue
      ...
      begin 
        Filevalue.transaction do
          filevalue.save
          #! should go the context upload action
          redirect_to(:controller => :contexts, :action => :upload, 
                      :filevalue_id => filevalue.id)
        end
      rescue ActiveRecord::RecordInvalid => invalid
        redirect_to :back
        #! showing the message that file upload did not work
      end
  end
  ...
  #! better would be if we could write "dataset_" in front of the
  #! files
  def freeformat_file_upload
    if logged_in?
      filevalue = Filevalue.new(params[:filevalue])

      begin 
        Filevalue.transaction do
          logger.debug "---- In freeformat_file_upload"
          filevalue.save

          # Creating the context
          filename = filevalue.file_file_name
          title = "Free format file " + filevalue.file_file_name
          abstract = "Dummy text for importing a free format file, please replace"
          filename_noending = filename.split('.')[0]
          export_filename = filename_noending + ".xls" # this is the ouput filename
          @context = Context.create(:title => title,
                                    :abstract => abstract,
                                    :filename => export_filename)

          # Creating the owner
          pers_role = @current_user.person_roles[0]
          logger.debug "pers_role.inspect"
          logger.debug pers_role.inspect
          @context.person_roles << pers_role
          logger.debug "@context.person_roles"
          logger.debug @context.person_roles.inspect

          # Creating the data column
          dc = MeasurementsMethodstep.new(:columnheader => "file",
                                          :definition => "free format file",
                                          :columnnr => 1)
          logger.debug "dc.inspect"
          logger.debug dc.inspect
          # Creating a new Data group by default
          dg = Methodstep.create(:title => title, :description => title)
          logger.debug "dg.inspect"
          logger.debug dg.inspect
          dc.methodstep = dg
          dc.context = @context
          dc.save
          ...
          # Adding file to sheet cell
          obs = Observation.create(:rownr => 2)
          cl = Measurement.create(:observation => obs,
                                  :value => filevalue,
                                  :measurements_methodstep => dc)

          redirect_to(:controller => :import, :action => :freeformat_context_edit, 
  ...
  # Directly after saving the general meta data of a meta data
  # spreadsheet in the ContextsController, all cell entries from the
  # raw data sheet are saved here without checking into instances of
  # Measurement.  This will later be mapped in value tables
  # (Datetimevalue, Numericvalue, Categoricvalue), but this needs user
  # interaction (raw_data_per_header).  Here, all data columns of the
  # spreadsheet are saved to instances of MeasurementsMethodstep and
  # these again are associated to data groups (Methodstep) by guessing
  # (find_similar_data_groups).  Provenance information for the data
  # columns is added in the following but separate step
  # (raw_data_per_header).
...
#    if logged_in?
    ...
    session[:return_to] = request.request_uri
    @context ||= Context.find(params[:context_id], :include => [:measurements_methodsteps])
   
    # import information from the spreadsheet
    filevalue = @context.upload_spreadsheet
    ...
    # Are there data columns already associated to this context?
    n_data_cols = @context.measurements_methodsteps.length
    ...
    if n_data_cols == 0 # which means that there are no observations
      ...
      if @checkUnique # we can only go on, if column headers of data
        ...
        @columnheadersRaw.each do |ch|
          ...
          data_group = Methodstep.find_by_title(data_group_ch[:title])
          if data_group.blank?
            logger.debug "- Creating new data group  -" 
            data_group = Methodstep.create(data_group_ch)
          end
          logger.debug data_group.inspect

          logger.debug "------------  update data column information ---------" 
          data_column_ch[:methodstep_id] = data_group.id
          logger.debug data_column_ch.inspect

          logger.debug "------------ create new data column ------- "
          data_column_new = MeasurementsMethodstep.create(data_column_ch)
          ...
          unless data_hash.blank?
            ...
            logger.debug "- Zeitchlucker?:  before @context.rownr_observation_id_hash -" 
            rownr_obs_hash = @context.rownr_observation_id_hash
            logger.debug "------------ after @context.rownr_observation_id_hash ---------" 
            logger.debug @context.rownr_observation_id_hash.inspect
   
            # Go through each entry in the spreadsheet
            data_hash.each do |rownr, entry|
              # Is there an observation in this context with this rownr?
...
#    end
  ...
  def raw_data_per_header
    ...
    @context ||= Context.find(params[:context_id], 
                              :include => [:measurements_methodsteps,
                                           :upload_spreadsheet])
    @data_column ||= @context.measurements_methodsteps.
      select{|dc| dc.columnheader == params[:data_header]}.first

    # open the spreadsheet
    filepath = @context.upload_spreadsheet.file.path
    provide_metasheets(filepath)
    ...
    if @ppl.blank?
      ...
      ppl.each do |person|
        person.has_role! :responsible, @data_column
      end
      @ppl = @data_column.people
    end
    ...
    @portal_cats = @data_column.methodstep.datacell_categories
    ...
    if @data_column.import_categories.blank?
      ...
      sheet_new_imp_cats = sheet_new_cats.
        map{|cat| ImportCategory.new(:categoricvalue => cat)}
   
      @data_column.import_categories = sheet_new_imp_cats
    end 

    @sheet_cats = @data_column.import_categories.
  ...
  def lookup_data_header_people(ch)
    ...
    people_rows.each do |r|
      ...
      people += Person.find_all_by_lastname(people_sur)
...
#  def dh_person_add_role
#    if logged_in?
#      data_header = MeasurementsMethodstep.find(params[:measurements_methodstep][:id])
#      person_role = PersonRole.create(params[:person_role])
#      data_header.person_roles << person_role
#      prs = data_header.person_roles.collect{|pr| pr.id}
##      session[:prs_ids][data_header.columnheader] = prs
# 
# 
#      if data_header.save
#        redirect_to :back
...
#  end
  ...
  def update_data_header
    if logged_in?
      ...
      if data_header.update_attributes(params[:measurements_methodstep])        
        redirect_to :back
      else
  ...
  def add_data_values
    if logged_in?
      data_column =
        MeasurementsMethodstep.find(params[:measurements_methodstep][:id])
      data_column.update_attributes(params[:measurements_methodstep])
  ...
  def update_data_group
    if logged_in?
      data_header = MeasurementsMethodstep.find(params[:measurements_methodstep][:id])
      data_group = Methodstep.new(params[:methodstep])
      ...
      begin 
        MeasurementsMethodstep.transaction do
          if data_group.save
            data_header.methodstep = data_group
            data_header.save
            redirect_to :back
          end
        end
  ...
  def update_import_category_change_category 
    if logged_in?
      icat = ImportCategory.find(params[:import_category][:id])
      exchange_cat = Categoricvalue.find(params[:import_category][:categoricvalue_id])
      oldcat = icat.categoricvalue
      icat.approved = true
      dh = icat.measurements_methodstep
      val = icat.raw_data_value
      # data cells with this raw data value have to get the new
      # category
      dcs = dh.measurements.select{|dc| dc.import_value == val}


      logger.debug "---  changing the category of an existing icat -----"
      logger.debug icat
      logger.debug params.inspect

      begin 
        ImportCategory.transaction do
          icat.categoricvalue = exchange_cat
          icat.save
          # add the new value to the cell values
          dcs.each do |dc|
            dc.value = exchange_cat
            dc.save
          end
          # get rid of unassociated categories
          oldcat.destroy
          redirect_to :back
        end
      rescue ActiveRecord::RecordInvalid => invalid
  ...
  def update_import_category_new_category 
    if logged_in?
      icat = ImportCategory.find(params[:import_category][:id])
      oldcat = icat.categoricvalue
      newcat = Categoricvalue.new(params[:categoric_value])
      icat.approved = true
      dh = icat.measurements_methodstep
      val = icat.raw_data_value
      # data cells with this raw data value have to get the new
      ...
      logger.debug "---  creating new category for an existing icat -----"
      logger.debug icat.inspect
      logger.debug params.inspect
      logger.debug dh
      ...
      begin 
        ImportCategory.transaction do
          ...
          # get rid of unassociated categories; will not destroy
          # categories linked to other cells or import_categories
          oldcat.destroy
          redirect_to :back
        end
      rescue ActiveRecord::RecordInvalid => invalid
  ...
  # Assembles information for a data column
  # (MeasurementsMethodstep). It uses information imported from the
  # metadata spreadsheet (provide_metasheets, methodsheet_datagroup).
  # It is called during the upload of the metadata spreadsheet
  # (raw_data_overview).  The new data column is by default linked to
  # the most similar data group (Methodstep) found on the portal
  # (find_similar_data_groups).  If no similar data group is found, a
  # dummy "Helper column" is linked.  A new data group can be created
  # during user interaction if this is not suitable.  This method uses
  # information imported from the metadata spreadsheet
  # (provide_metasheets, methodsheet_datagroup).  Provenance
  # information is not yet included in this step.
  def data_column_info_for_columnheader(columnheader, methodsheet,
    ...
    data_header_ch = {:context_id => data_set_id,
      :columnheader => ch,
  ...
  # tables linked.  This should be similar to the ".to_label" methods that
  # active scaffold is using.  Each value table should have an own way how to
  # present itself
  def submethod_summary(submethod)
    submethod_mms = submethod.measurements
    if submethod_mms.length > 0
      submDataType = submethod_mms[0].value_type
    else
      submDataType = "No Measurements provided for this column yet"
    end

    # get the meaningful values
    if submDataType == "Categoricvalue"
      submData =  submethod.measurements.collect{|m| m.categoricvalue.long}
    end
    if submDataType == "Numericvalue"
      submData =  submethod.measurements.collect{|m| m.numericvalue.number}
    end
    submData = submData.compact.uniq

    @submethodSummary = {:submeth => submethod,
      :datatype => submDataType, :data => submData
    }
  end

  # Assembles data from the raw data sheet for a given columnheader
  # (data column, MeasurementsMethodstep) of the raw data sheet.
  # Determines the maximum row number after deleting empty cells.  The
  # row numbers stored in the ":data" hash are equal to the row
  # numbers of the spreadsheet (starting with "2", since the first row
  ...
  def load_data_column(co, ch)
    col = Array(@rawdatasheet.row(0)).index(ch) # begin at 0
    measmeth = co.measurements_methodsteps.find_by_columnheader(ch)
    p measmeth.columnheader + " " + col.to_s
    ## load the data
    cldata = Array(@rawdatasheet.column(col))
    cldata.delete_at(0) # first entry is columnheader
    ## return data
    return cldata
  end
  ...
  ## check if data has been included as in the data sheet; numeric data;
  ## after including the values
  def check_values(co, ch, cldata, datatype, first, last)
    co = Context.find(co.id)
    measmeth = co.measurements_methodsteps.select{|mm| mm.columnheader == ch}.first
    ms = measmeth.measurements
    (first..last).each do |i|
      ...
      unless n.nil?
        case datatype
        when "number"
          p ms.select{|m| m.rownr==(i+1)}.first.value.number
        when "cat"
          p ms.select{|m| m.rownr==(i+1)}.first.value.short
          p ms.select{|m| m.rownr==(i+1)}.first.value.long
          p ms.select{|m| m.rownr==(i+1)}.first.value.description
        end
      end
      p "-------------"
    end
  ...
  def category_data_column_import(data_column_id, portal_cats, sheet_cats)
    data_column = MeasurementsMethodstep.find(data_column_id, 
                                              :include => :measurements)

    # the entries themselves
    cells = data_column.measurements
  ...
  def data_column_categories
    @data_column = MeasurementsMethodstep.find(params[:data_column_id])
    @context = @data_column.context
    portal_cats = @data_column.methodstep.datacell_categories
    sheet_cats = @data_column.import_categories.map{|icat| icat.categoricvalue}
    @cats_to_choose = [portal_cats + sheet_cats].flatten.uniq
    @cats_to_choose.sort!{|x,y| x.verbose <=> y.verbose}
    cells_with_cats = @data_column.measurements.
  ...
  def cell_category_create
    if logged_in?
      first_cell = Measurement.find(params[:measurement][:id])
  ...
  def cell_category_update
    if logged_in?
      ...
      logger.debug params[:measurement].inspect
      first_cell.update_attributes(params[:measurement])
      same_entry_cells = first_cell.same_entry_cells
