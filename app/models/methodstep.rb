...
class Methodstep < ActiveRecord::Base
  has_many :measurements_methodsteps
  ...
  def datacell_categories
    dcs = self.measurements_methodsteps.collect{|dh| dh.measurements}.flatten
  ...
  def datacell_importcat_categories
    cats1 = self.datacell_categories
    cats2 = self.importcat_categories
    cats = (cats1 + cats2).uniq
    cats = cats.sort{|x,y| x.short <=> y.short}
  end


  def importcat_categories
    icats = self.measurements_methodsteps.collect{|dh| dh.import_categories}.flatten
    cats = icats.collect{|icat| icat.categoricvalue}.uniq
    cats = cats.sort{|x,y| x.short <=> y.short}
  end
