class DataRequest < ActiveRecord::Base
  ...
  belongs_to :corresponding, :class_name => "Person", :foreign_key => "corresponding_id"
  belongs_to :senior_author, :class_name => "Person", :foreign_key => "senior_author_id"

  has_many :authors, :class_name => "Person", :source => :person, :through => :author_data_requests
  has_many :author_data_requests, :dependent => :destroy, :include => [:person]

  has_many :coordinators, :class_name => "Person", :source => :person, :through => :data_request_votes,
  ...
  has_many :data_request_contexts, :dependent => :destroy
  has_many :contexts, :through => :data_request_contexts
